...
with `node tools/bench-model.mjs`, delete the `perf-baseline` branch so CI measures afresh, and say why in the
commit.

The baseline is `baseline.json` on the `perf-baseline` branch, whose history is the record. Nobody edits it:
...
Locally, `yarn perf` prints the same table against that file (through `gh`, if you are logged in) with looser
