...
| Path | What |
|---|---|
| `packages/core` | schema, validator, declaration, view scoping, renderer, packs, shapes. `src/index.ts` is the public API, pinned by `test/api.test.ts`; `src/internal.ts` and `src/testing.ts` are the unpromised subpaths; `src/generated.ts` is written by `scripts/embed.mjs` from the schema and the built-in pack before every build, so the root runs in a browser |
...
| `packages/pack-aws`, `pack-azure`, `pack-gcp` | the providers' icon packs, each under the provider's own terms |
...
The cloud packs are the providers' own icon sets, generated by `node tools/packs/build.mjs` and committed, each as
its own package `packages/pack-<name>` with the provider's terms as its LICENSE, never inside an MIT package. Their
terms are in [docs/PACKS.md](docs/PACKS.md); do not add a set whose terms do not permit redistribution for
architecture diagrams. A new pack package needs its trusted publisher on npm before the next release.
