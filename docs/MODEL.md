...
| `collapse` | array of group id | | Groups drawn closed in this view, as a level of detail: the group keeps its real footprint, its name and a count sit centred on it, its members and internal connections are hidden, and connections into it stop at its frame. All of that reverses while the group is in focus, from a scene or a click. Health is unchanged (R11). |
...
| R11 | A collapsed group is a level of detail on one layout: its frame keeps its footprint, a centred summary carries its name and count, members and internal connections are present but hidden, and connections into it are also drawn cut at its frame for the summary level. Focusing the group reverses the two levels; nothing moves. The runtime focuses on click with its camera, and Escape returns. | view.test "collapsed groups (R11)"; render.test "level of detail (R11)"; boot.test "drill-down" |
| R12 | A tour whose scenes share one view is one drawing: state layers only where the scenario moment differs, a camera track that closes on each scene's focus group with that group as the fixed point, level-of-detail tracks that reveal a group while it is in focus, and a caption per scene. Scenes across views crossfade between whole views. Frame tooling inspects the first state at the identity camera; the runtime plays the scenes with its camera and the scenario player, stopped by the first interaction. | render.test "tour of views (R12)"; raster document.test "tours"; boot.test "runtime tour" |
