...
| `tour` | `{ seconds, scenes }` or `{ seconds, views }` | | A story told on a timer. Each scene is a view at a moment: `{ view, focus?, scenario?, step?, set?, note?, seconds? }`; `focus` opens a closed group and closes the camera on it; `views` is shorthand for one scene per view. When every scene shares one view, the file is one drawing that moves between layouts in CSS, so it plays inside an image tag; the runtime does the same until the reader interacts (R12). |
...
| `focus` | A group in the view's `collapse` to draw open, with the closed groups above it: a still of the inside. |
...
| S16 | Export ids are unique; `view`, `scenario`, `step`, `set` and `play` resolve as elsewhere; `focus` is a group in the view's `collapse`; `play` and `scenario` are exclusive; `tour` needs a tour and stands alone. | validate.test "exports (S16)"; invalid fixtures `export-duplicate-id`, `export-unknown-view`, `export-focus-not-closed`, `export-play-and-scenario`, `export-tour-without-tour` |
...
| R11 | A closed group is drawn as one box the size of a component: its name centred, an expand mark, nothing inside. Its members and everything below them leave the view; connections to them re-attach to the box, and several onto one pair of boxes merge into one line with their loads summed. Every set of open groups is its own layout. Opening a group (a scene's focus, or a click) moves the picture to that layout: shared entities slide, frames resize, edges are swapped, what appears fades in once settled. Closed groups nest to any depth; opening one opens every group above it; Escape steps back out one level. The interactive file carries one layer per group a reader can open. | view.test "collapsed groups (R11)"; render.test "closed groups (R11)"; boot.test "drill-down" |
| R12 | A tour whose scenes share one view is one drawing that moves. Each scene opens the groups its focus needs, every distinct set of open groups has its own layout, and the drawing holds the union of entities: each has a position track, a visibility track and one variant per state it takes; a group's frame also has a size track and swaps its title for its summary. Edges are one set per distinct drawing. A transition has three phases that never overlap: what leaves fades (caption, entities, edges), then positions, sizes and the camera ease to the new layout, then what arrives fades in. States crossfade over the whole window. The camera closes on the focus in its scene's layout, clipped to the stage; the legend and caption are a fixed strip below. Everything shown per scene carries whether it is visible at t = 0, so a still keeps exactly the first scene. Scenes across views crossfade between whole views. | render.test "tour of views (R12)"; raster document.test "tours"; boot.test "runtime tour" |
