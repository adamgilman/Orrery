...
| `tour` | `{ views, seconds }` | | Show those views in turn, `seconds` each (default 4), looping. In the file, a CSS crossfade between complete layers, so it plays inside an image tag; in the runtime, the morph, until the reader interacts (R12). |
...
| R12 | A tour is one complete layer per view on the largest size, crossfaded by CSS with the declared period, each captioned with the view's title; frame tooling inspects the first; the runtime tours with the morph, stopped by the first interaction. | render.test "tour of views (R12)"; raster document.test "tours"; boot.test "runtime tour" |
