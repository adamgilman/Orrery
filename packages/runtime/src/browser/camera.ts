...
export interface Frame { left: number; margin: number; maxZoom?: number; pad?: number }

/** Scale and translate so `view` fits the screen area right of the panel, centred. */
export function fitView(view: Size, screen: Size, f: Frame): Camera {
  const freeW = screen.width - f.left - 2 * f.margin, freeH = screen.height - 2 * f.margin;
  const k = Math.min(freeW / view.width, freeH / view.height, f.maxZoom ?? 1.5);
  return { k, tx: f.left + f.margin + (freeW - view.width * k) / 2, ty: f.margin + (freeH - view.height * k) / 2 };
}
...
export function zoomToBox(box: Box, screen: Size, f: Frame): Camera {
  ...
  return { k, tx: f.left + f.margin + freeW / 2 - cx * k, ty: f.margin + freeH / 2 - cy * k };
}
