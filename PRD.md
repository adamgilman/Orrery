...
| Heading block | `description` on the model and on a view; `heading: true` on an export or `render --heading` draws title and description above the picture, still or interactive (the runtime's camera works below it). |
